...
bool CSyndicateWorld::Create(IRecordset* pRes)
{
	...
	sprintf(szSQL, "SELECT COUNT(*) FROM %s as s LEFT JOIN %s AS u ON u.id=s.id WHERE s.owner_id=%u && u.id IS NOT NULL", _TBL_SYNMEMBER, _TBL_USER, m_pData->GetID());
	IRecordset* pStatRes = GameWorld()->GetDatabase()->CreateNewRecordset(szSQL);
	if(pStatRes)
	{ //This column is not correct in synmember
...
int CSyndicateWorld::AppendMember(class CMsgNameW& msg, int idx, int idxFirst, int& nCount)
{
