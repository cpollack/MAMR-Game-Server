...
//		return false;
	...
	{
		int	nPower = 0; // CBattleSystem::CalcPower(HitByMagic(), m_pOwner, pTarget, GetPower(k));
		...
		case	STATUS_ATTACK:
			{
				...
				nPower = 0; // CBattleSystem::CalcAttackPower(m_pOwner, pTarget, &nAdjustAtk, 0);
			}
			break;
		...
		case	STATUS_DEFENCE3:
			{
				...
				nPower = 0; // CBattleSystem::CalcAttackPower(m_pOwner, pTarget, 0, &nAdjustDef);
			}
			break;
...
//			break;
		case	STATUS_ATKSPEED:
			{
				int	nPause	= GetPower();
				nPower = 0; // CBattleSystem::CalcAttackPower(m_pOwner, pTarget);
		...
		case	STATUS_VAMPIRE:
			{
				...
				nPower = 0; // CBattleSystem::CalcAttackPower(m_pOwner, pTarget, &nAdjustAtk);
	...
	for (int k=0; k<m_nApplyTimes; k++)
	{
		...
		IF_OK(msg.CreateByPos(m_pOwner->GetID(), m_pMagic->GetInt(MAGICDATA_TYPE), m_pMagic->GetInt(MAGICDATA_LEVEL), pos.x, pos.y, m_pOwner->GetDir()))
		{
			for(int i = 0; i < setRole.size(); i++)
			{
				...
				if (pRole && pRole->IsAlive())
				{
					int nPower = 0; // CBattleSystem::CalcPower(HitByMagic(), m_pOwner, pRole, GetPower(k));
					setPower.push_back(nPower);
...
bool CMagic::ProcessFan()
{
	...
	if (pStatus)
	{
		int nLifeGot = ::CutTrail<int>(0, MulDiv(nPowerSum, pStatus->GetPower(), 100));
		if (nLifeGot > 0)
			m_pOwner->AddAttrib(_USERATTRIB_LIFE, nLifeGot, SYNCHRO_TRUE);
	}
	AwardExp(nExp);
...
//		return false;
	...
	if(pTarget)
	{
		nPower = 0; // CBattleSystem::CalcPower(HitByMagic(), m_pOwner, pTarget, GetPower());
	}
	...
	for(int i = 0; i < MAX_SERIALCUTSIZE; i++)
	{
		...
		for(int j = 0; j < setTarget.size(); j++)
		{
			...
			if(setTarget[idx])
			{
				...
				if(setTarget[idx]->QueryRole(IPP_OF(pRole)) && pRole->IsAlive() && !IsImmunity(pRole))
				{
					int	nPower = 0;// CBattleSystem::CalcPower(HitByMagic(), m_pOwner, pRole, GetPower());
