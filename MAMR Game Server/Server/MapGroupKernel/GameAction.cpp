	...
	case	ACTION_MST_DROPITEM:
		{
			...
			else if (0 == _stricmp(szCmd, "droptrap"))//monster被杀死后掉陷阱, param "droptrap traptype lifeperiod"。
			{
				/*OBJID idTrapId=NULL;				
	...
	case	ACTION_NPCONLY_DELETE_PET:
		{
			...
			for(IRoleSet::Iter i = RoleManager()->QuerySet()->Begin(); i != RoleManager()->QuerySet()->End(); )		// 删除
			{
				IRole* pRole = RoleManager()->QuerySet()->GetObjByIter(i);
				i++;		//??? 提前移动，下面可能会删除当前角色
				CAiNpc* pPet;
	...
	case	ACTION_SYN_ATTR:
		{
			...
			if (_stricmp(szField, "money") == 0)//基金
			{
				if(strcmp(szOpt, "+=") == 0)
				{
			...
			else if (_stricmp(szField, "repute") ==0)//声望
			{
				if(strcmp(szOpt, "+=") == 0)
				{
			...
			else if (_stricmp(szField, "membernum") ==0)//成员人数
			{
				if(strcmp(szOpt, "<") == 0)
				{
