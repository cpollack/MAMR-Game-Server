...
bool CMyRecord::Open(CMyRecordset  *pMyRecordset, DWORD idNewKey /*= NEWKEY_NONE*/)
{
	...
	for(int i = 0; i < (int)m_uiFieldsCount; i++)
	{
		...
		//*(m_Fields + (sizeof(MYSQL_FIELD) * i)) = pMyRecordset->m_Fields[i];
		m_Fields[i] = pMyRecordset->m_objFields[i].m_Field;
...
#else
		m_Fields[i].name = (char*)g_heapSystem.New(sizeof(char) * (strlen(pMyRecordset->m_objFields[i].m_Field.name)+1));
#endif
		strcpy(m_Fields[i].name, pMyRecordset->m_objFields[i].m_Field.name);		//VVVVV 注意删除。table、def不复制，从未用到过
	}
