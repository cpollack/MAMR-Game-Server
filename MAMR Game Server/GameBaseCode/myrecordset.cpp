...
bool CMyRecordset::GetDefauldField()		//paled - 取缺省域值，并填入objField的VAL联合中。
{
	...
	query += m_szTableName;
	mysql_query(m_pMyDatabase->m_hdbc, query.c_str());
	
	MYSQL_RES *pRes = mysql_store_result(m_pMyDatabase->m_hdbc);
	...
	for(int i = 0; i < FieldsCount(); i++)
	{
		...
		case	FIELD_TYPE_SHORT:
			if(pRow[4])
				m_objFields[i].m_iVal = atol(pRow[4]);
			else
				m_objFields[i].m_iVal = 0;
			break;
		case	FIELD_TYPE_LONG:
			if(pRow[4])
				m_objFields[i].m_lVal = atol(pRow[4]);
			else
				m_objFields[i].m_lVal = 0;
			break;
		case	FIELD_TYPE_DECIMAL:
		...
		case	FIELD_TYPE_DOUBLE:
			if(pRow[4])
				m_objFields[i].m_dVal = atof(pRow[4]);
			else
		...
		case	FIELD_TYPE_VAR_STRING:
			if(pRow[4])
				strcpy(m_objFields[i].m_szVal, pRow[4]);
			else
...
//@@@int sec = (rand()%5)*100 + 0;printf("*[%d] --> ", sec/100); Sleep(sec); printf("|||\n");
		...
		{	
			...
			{	
				...
				for (UINT i =0; i< m_uiFieldsCount; i++)
				{
					m_Fields[i] = *mysql_fetch_field(m_Recordset);
