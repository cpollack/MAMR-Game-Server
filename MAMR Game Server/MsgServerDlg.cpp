...
/*#if	!defined(PALED_DEBUG)
#ifdef _DEBUG
	this->MessageBox("This version of the server side can only run internally, and it is absolutely forbidden to run externally!", "Warning", MB_OK|MB_ICONEXCLAMATION);
#endif
#endif*/
	...
	m_hMutexServer = ::CreateMutex(NULL, false, "MAMRServer");
	if (m_hMutexServer)
	{
...
//*
				...
				for( i = 0; i < m_setMapGroupThread.size(); i++)
